      ...
      >
        {data.map((row, r) => {
          return row.map((column, c) => {
            let top: CSS.Property.BorderTopWidth = TOP_LEFT.includes(r)
              ? '3px'
              : '1px';
            let right: CSS.Property.BorderRightWidth = BTM_RIGHT.includes(c)
              ? '3px'
              : '1px';
            let btm: CSS.Property.BorderBottomWidth = BTM_RIGHT.includes(r)
              ? '3px'
              : '1px';
            let left: CSS.Property.BorderLeftWidth = TOP_LEFT.includes(c)
              ? '3px'
              : '1px';
            return (
              <div
